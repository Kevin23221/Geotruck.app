...
def agregar_ruta(request):
    if request.method == 'POST':
        ...
        if origen and destino and distancia:
            Ruta.objects.create(origen=origen, destino=destino, distancia_km=distancia,
                                descripcion=descripcion, creada_por=request.user)
        ...
        if placa and marca and modelo:
            if Vehiculo.objects.filter(placa=placa).exists():
                ...
                Vehiculo.objects.create(placa=placa, marca=marca, modelo=modelo,
                                        destino_asignado=destino, conductor=conductor, foto=foto)
...
def mapa_view(request):
    ...
    for ruta in rutas_activas:
        if ruta.lat_actual is not None and ruta.lng_actual is not None:
            ubicaciones.append({
                "lat": ruta.lat_actual, "lng": ruta.lng_actual,
...
def viaje_editar(request, viaje_id):
    v = get_object_or_404(Viaje, id=viaje_id)
    if request.method == 'POST':
        v.conductor_nombre   = request.POST.get('conductor_nombre', '')
        v.conductor_licencia = request.POST.get('conductor_licencia', '')
        v.conductor_cedula   = request.POST.get('conductor_cedula', '')
        v.placa              = request.POST.get('placa', '')
        v.modelo             = request.POST.get('modelo', '')
        v.fecha_salida       = request.POST.get('fecha_salida') or None
        v.fecha_llegada      = request.POST.get('fecha_llegada') or None
        v.origen             = request.POST.get('origen', '')
        v.destino            = request.POST.get('destino', '')
        v.km_recorridos      = request.POST.get('km_recorridos') or None
        v.tipo_carga         = request.POST.get('tipo_carga', '')
        v.recursos           = request.POST.get('recursos', '')
        v.observaciones      = request.POST.get('observaciones', '')
        # Reemplazar gastos
        v.gastos.all().delete()
        descs  = request.POST.getlist('gastos_descripcion[]')
        cats   = request.POST.getlist('gastos_categoria[]')
...
def conductores(request):
    lista = Conductor.objects.all().order_by('-fecha_ingreso')
    return render(request, 'conductores.html', {'conductores': lista})
...
def agregar_conductor(request):
    if request.method == 'POST':
        ...
        if nombre and cedula and licencia:
            if Conductor.objects.filter(cedula=cedula).exists():
                ...
                messages.success(request, f'Conductor {nombre} registrado correctamente.')
        else:
            messages.error(request, 'Nombre, cédula y licencia son obligatorios.')
